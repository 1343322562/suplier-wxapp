...
export const printContentHandle = function (data, type) {
  ...
  data.forEach((item, index) => {
    ...
    item.details.forEach((good, i) => {
      ...
      content += `<N> <BOLD>${contentHandle.unit(good.realQty+good.unitNo, 7)}</BOLD>${contentHandle.unit(good.OrgiPrice.toFixed(2), 10)}<BOLD>${contentHandle.unit(good.subAmt.toFixed(2)+'/'+good.unitNo, 11)}</BOLD>${contentHandle.unit((good.subAmt*good.realQty).toFixed(2), 10)}${('warehouse' in item && item.warehouse.length > 3 && item.warehouse.silce(0, 3)) || '空'}</N><BR><BR>`
    })
    content += `<C><N>————————————————————————</N><BR></C>`
