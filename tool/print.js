...
export const printContentHandle = function (data, type) {
  ...
  data.forEach((item, index) => {
    ...
    item.details.forEach((good, i) => {
      ...
      content += `<N> ${contentHandle.unit(good.realQty+'/'+good.unitNo)}${contentHandle.unit(good.subAmt.toFixed(2), 10)}${contentHandle.unit(good.subAmt.toFixed(2)+'/'+good.unitNo)}<BOLD>${contentHandle.unit((good.subAmt*good.realQty).toFixed(2), 10)}</BOLD>${item.warehouse || '空'}</N><BR>${item.details.length == 1 ? '' : '<BR>'}`
    })
    content += `<C><N>————————————————————————</N><BR></C>`
